...
def convert(
    ...
    colors = {item['text']: item['backgroundColor'] for item in json.load((path / 'tags.json').open())}
    ...
    for week_path in result:
        ...
        for annotators in annotated_pairs:
            ...
            assert all(annotators[0]['text'] == d['text'] for d in annotators),\
            ...
            for annotator_id, annotation in enumerate(annotators, start=1):
                ...
                spans = (doc.char_span(*s, alignment_mode=token_alignment) for s in annotation['label'])
                ...
                doc.spans['annot%d' % annotator_id] = [s for s in spans if s is not None]
...
def create(
    ...
    annotation.merge(docs, output='all', scorer=span_scorer)
    annotation.categorize(docs, span_key='all')
    return docs
